...
export default function StoreDirectoryPage({ breadcrumbItems, categories: initialCategories, stores, searchValue, countryCode = DEFAULT_COUNTRY_CODE, t: propT }) {
            ...
            <div className="relative w-full h-full">
              ...
              <div className="absolute right-4 top-2 w-[200px] h-[110px] rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-md p-4 rotate-6 shadow-[0_20px_40px_rgba(139,92,246,0.25)]">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[7.5px] font-black uppercase tracking-wider text-[var(--color-primary)]">{t.verifiedPartner}</span>
                </div>
                <div className="mt-2.5 text-lg font-black text-white tracking-wider">STORES</div>
                <div className="mt-0.5 text-[8.5px] font-semibold text-white/40">{t.verifiedCoupons}</div>
                <div className="mt-3 pt-1.5 border-t border-white/5 flex items-center justify-between">
                  <span className="text-[7.5px] font-bold text-white/30">{t.updated24h}</span>
              ...
              <div className="absolute left-2 bottom-1 w-[180px] h-[95px] rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-md p-3.5 -rotate-6 shadow-[0_20px_40px_rgba(139,92,246,0.25)]">
                <div className="text-[7.5px] font-black uppercase tracking-wider text-[var(--color-primary)]">{t.totalCatalog}</div>
                <div className="mt-1 text-[13px] font-black text-white tracking-tight">{t.activeBrands}</div>
                <div className="mt-2.5 pt-1.5 border-t border-white/5 flex items-center justify-between">
                  <span className="text-[7.5px] font-semibold text-white/30">{t.topSavings}</span>
          ...
          <div className="relative z-10 max-w-2xl lg:max-w-[640px]">
            ...
            {hasSearch && (
              <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-md px-5 py-4 max-w-md shadow-lg">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-primary)]">{t.activeQuery}</p>
                <p className="mt-1.5 text-sm text-white/95">
                  {t.showingResultsFor} <span className="font-bold text-[var(--color-primary)]">&quot;{trimmedSearch}&quot;</span>
                </p>
              </div>
            )}
          </div>
            ...
            {paginatedStores.length ? (
              ...
              <div className="rounded-[28px] border border-[var(--border)] bg-[var(--surface)] p-10 text-center">
                <p className="text-xl font-semibold text-[var(--text)]">
                  {hasSearch ? t.noStoresFound.replace("{query}", trimmedSearch) : t.noStoresAdded}
                </p>
